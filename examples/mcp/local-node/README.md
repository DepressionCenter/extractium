...
That package brings a machine-learning runtime with it, and `npm audit` reports advisories in two of its own dependencies that have no fix available: `sharp` (image decoding) and `adm-zip` (unpacking the runtime's binaries at install time). Neither path is used by this server, which reads one file and embeds one sentence, but the advisories are real and worth knowing about before you install. If you would rather not, use the [Python server](../local-python/README.md): it embeds with the package Extractium already installs.
