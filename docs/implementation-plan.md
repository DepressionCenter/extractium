...
*Finished 2026-09-17 on branch `llms-index-tree`. Built as written, with four additions that real data called for. A repository's root README is listed ahead of its other root files. A label shared by a website source and a GitHub source, which the center's settings file has, lists the website's pages and then the repositories in one file. A keyword list that more than three pages of one source share gives way to the keywords of the page's own first section: a repository's topics are tags on every one of its files, so the first rendering described 700 files of one repository with the same line. And a page record keeps the summary its source gave beside the one the shared-summary rule may blank, because a site's default description is the right description of the site and of a repository. Measured by rendering the tree from the sections the 2026-09-16 build stored, with the descriptions now in `examples/config.efdc.yaml`, which gives the same files as a rebuild without repeating the embedding pass: `llms.txt` is 3,120 bytes and 7 entries, where it was 3,517,083 bytes and 6,884 lines; the tree is 46 files and 924,254 bytes in all, where `llms.txt` and `llms-full.txt` together were 31,356,330 bytes; the largest file, the first 500 files of one repository, is 183,211 bytes; no file holds more than 500 page entries and one continuation link; every relative link between index files resolves; and no code record is listed. Of the 2,723 pages listed, 1,172 are described by their own summary, 1,546 by keywords, and 5 by the opening of their text. A file of 500 entries is about 45,000 tokens, which is more than some browsing tools keep, so the cap, `MAX_ENTRIES_PER_FILE`, is the first thing to lower if a platform is seen to cut these files short.*
...
*Finished 2026-09-17 on branch `light-and-full-containers`. Built as written, with three changes. First, the light compendium is not made through the ordinary build step. That step drops a section shorter than 60 characters and collapses near-identical windows, and a light section is often both: `Keywords: sleep, wearables.` is 27 characters, and two pages of one site can share a description. `extractium/core/light.py` therefore builds the light sections, their windows, and their statistics directly, so every page stays in the file. A light section keeps the identifier of its page's first section. Second, the function that names the full file lives in the container adapter and not in the configuration module, because no adapter imports the configuration. Third, an output that names its own `file` and says nothing about `gzip` is compressed only when the name ends in `.gz`, so a settings file written before this phase that says `file: kb.json` still gets a plain file under that name. Measured on the center's build of 2026-09-16, by reading its container back and writing the two new files from it: the old `efdc-compendium.json` was 104,203,062 bytes with 27,491 sections and 76,496 windows. The light file is 1,293,140 bytes compressed and 4,449,977 inflated, with 2,723 sections and 3,002 windows, which is under the 10 MB a free Val Town account stores. The full file is 26,670,269 bytes compressed and 82,057,537 inflated, with 20,877 sections and 59,251 windows; the 6,614 code sections are what it no longer holds. Describing and embedding the 2,723 pages took 58 seconds on a laptop processor, the model's loading included, and writing both files took 9. With the relevance cut-off turned off, the full file returns the same five sections in the same order as the old file for "sleep data" and for "peer to peer", and the light file returns the pages one would want: the Sleep Data Automation article, repository, and slides for the first, and the Peer-to-Peer conference pages and resource site for the second. With the cut-off on, the old file and the full file both return nothing for those two short queries, because this corpus calibrates to a mean best-match similarity of 0.93. That is how the search behaved before this phase, it is not changed here, and it is worth its own look. A page on the SQLite tables, with a diagram and sample queries, was added in the same change because the database is now the only single file that holds code analysis. The README diagram and the two workflow images still printed `llms.txt / llms-full.txt`; that label now reads `llms.txt / llms/` in the PNG files and in both SVG files, rebuilt from letters already in each image. Their search-index label still reads `compendium.json`, because the letters of `.gz` are not in the artwork and its font was not at hand.*
...
**PDF reader.** Issue #57, finished 2026-09-15 on branch `feat-pdf-reader`. The document readers gained PDF, read with `pypdf` (the optional `pdf` extra, carried by the lock file) in a child process with a time limit per file, so a malformed file is skipped rather than allowed to stall a build. Headings come from a file's bookmarks with the page number appended, or from the page numbers, so each section of the index names its page; the title, subject, and keywords come from the document properties. In the configuration reference under "Reading document files".

**Single pages on other hosts.** Issue #54, finished 2026-09-15 on branch `feat-leaf-patterns`. A `leaf_patterns` list on a `web` source names pages on other hosts that the site links to; each is fetched and indexed when a page in the seed's scope links to it, and its own links are never followed, so the crawl never spreads to that host and the automatic scope stays as it is. The Peer-to-Peer site in the center's settings file uses it to reach its shared Google files and the Word files on its delivery network. In the configuration reference under "Single pages on another host".

**Shared Google files and document readers.** Issues #56 and #58, finished 2026-09-15 on branch `feat-document-readers`. A `google_docs` site handler reads a Google Docs, Sheets, or Slides file shared with the link through its export, under one address whatever link shape reached it; a file that is not shared is reported and skipped. A standard-library reader turns Word, OpenDocument, and RTF files into text with their headings and their own keywords and description, behind a `read_documents` setting on the `web`, `local`, and `github_api` sources, off by default. The site-handler protocol gained the optional `canonical_url` and `landing_allowed` hooks. Both are in the configuration reference.
...
**Audio fallback for refused caption requests.** Finished 2026-09-16 on branches `whisper-by-default-and-linked-video-report` and `audio-pacing-and-block-report`. YouTube refuses caption requests to most machines that build on a schedule and to many on shared networks, so a build could index no video at all from such a machine. When a caption request is refused, the video's audio is downloaded with `yt-dlp` and transcribed on the CPU with Whisper's smallest English model through `faster-whisper`, and the transcript is stored like any other. Both packages are the `whisper` extra and are pinned in the lock file, so every scripted build has them. Downloads after the first wait a random few seconds, and the summary says whether YouTube refused the captions, the audio, or both. `audio_fallback: false` on a source switches it off. In the configuration reference under "When YouTube refuses the machine".

**A relevance floor on the raw cosine, in place of the calibration cutoff.** Finished 2026-09-17 on branch `raw-cosine-relevance-floor`. Both clients and the Cloudflare example required a result to beat `calibration.mean + 1 x std`. That figure measures how similar each window is to its nearest neighbouring window, 0.93 on the center's build and 0.92 on a 4,592-window build of this project's own documentation, and it was compared with a fused rank score that tops out at 0.98 or with a query cosine that runs from 0.70 to 0.89 for a good match. On the center's full container 2 of 40 answerable test queries returned anything, and 2 of 25 unrelated ones did, because passing depended on one window ranking first in both lists. Removing the term alone let every unrelated query through. Every candidate now carries its raw cosine to the query, and must reach 0.67, the floor Field Station AI measured and adopted separately the same day; `calibration` is no longer read. With the change the same queries score 40 of 40 and 1 of 25 on the full container, and 40 of 40 and 0 of 25 on the light one. The committed contract query is now a blend of two windows, so the snapshot holds two sections above the floor and the rest of the pool below it. Still open from the same investigation: a floor measured per compendium at build time, because a fixed one is too strict for a small light container (6 of 25 on a 28-page test build) and loosens as a corpus grows. The two chunker defects the same investigation found are the next entry.

**A page is indexed once, and long sections are cut between words.** Finished 2026-09-17 on branch `chunk-pages-once`. The chunker gathered the text before a page's first heading from the content node's direct children, and stopped only at a direct child that was a heading. Most sites put an article's headings inside a wrapper element, so the loop never stopped, the whole page became "before" text, and the page was indexed once as page-level slices and once as its sections. Near-duplicate collapse never removed the copy, because it never collapses two windows of one page. In the center's full container of 2026-09-16 that was 5,594 of 20,877 sections and a third of the text, and a search often returned a page-level slice beside the sections it repeated. Section text is joined with spaces and so has no line breaks, which made the old rule cut every long section at exactly 1,200 characters: 6,536 sections, 5,248 of them ending inside a word. The page is now walked in document order and divided at each heading at any depth, and a long section is cut at a line break, else a sentence end in the second half of the limit, else a space. Splitting the 1,648 cached HTML pages of the center's build both ways, without embedding: 14,553 sections and 12,137,981 characters before, 9,467 sections and 6,097,887 characters after, none cut inside a word. Every one of the 873,215 words in those pages' content lands in exactly one section, apart from 3,097 in sections under the 60-character minimum, which were dropped before too. Both behaviors came from the frozen reference script, whose tests still pin them for the reference; the comparison tests now ignore the doubled spaces the reference leaves where whitespace sat between elements. Section identifiers on pages with nested headings change, because the page-level slices that shared the bare title as a heading are gone; a section under its own heading keeps its identifier. The contract fixture's long page repeated one sentence, which the sentence cut turned into identical windows that the two clients ordered differently, so its sentences are now numbered.
...
**Speech-to-text for videos without captions.** Decided on 2026-09-11, on evidence rather than on principle, and still the rule for a video that has no captions: it is skipped and counted. Sampling 65 of the 145 videos on a real channel found English captions on 64 of them, 37 of those generated by YouTube itself, and not one video with captions missing or turned off. The one gap was a video captioned in Spanish only, which is a language to add to the `languages` setting and not a model to run. What that sample did not show is how often YouTube refuses the machine that asks for the captions. That turned out to be the common failure, and it is answered by the audio fallback listed under "Done since" above: a video is transcribed from its audio when the caption request is refused, never because its captions are absent.

**Video from anywhere but YouTube.** The same reasoning fixes the scope of video indexing: captions from YouTube, and no other video host or local media file. Reading video from anywhere else means fetching and decoding media from hosts and files this project knows nothing about, which is a class of malformed-input risk it takes on in one place only, the YouTube audio fallback, and it buys nothing for the documentation a knowledge base is built from. An organization that needs it can write a source plugin, which is what the plugin registry is for.
