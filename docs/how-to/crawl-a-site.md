...
build has to run on your own computer.
...
This page takes you from an installed tool to a settings file you trust. You choose a source type for each kind of content, run a small trial, read what it indexed, tighten the URL patterns, and repeat until the page list is right. It also explains every notice a build prints, so nothing in the log is a mystery. You do not need to write code. [Running a build](../usage.md) is the command reference, and the [configuration reference](../configuration.md) holds every setting.
...
Limit the first run so a pattern broader than you meant costs seconds rather than an hour:
...
2. Are pages you expected missing? Raise the limit a little and look again before changing anything else. Twenty-five pages is not many.
...
Four lists on a `web` source decide what is fetched and what is indexed. Each holds regular expressions matched against the whole URL.
...
| `leaf_patterns` | Single pages on other hosts that the site links to, such as shared Google files or handouts on a file host. Each is fetched and indexed, and its links are never followed. The scope from the seed stays as it is. | Empty. |
...
|---|---|---|
| `GITHUB_TOKEN` | A build reads more than a few GitHub repositories, or runs often enough to reach the public request limit. | The same content is read through the public API, at a lower request limit. The build reports which tier served each repository. |
...
| `SKIP <url> -- it redirects to <address>, ... so what arrived is not this site's content` | A page inside the site sent the crawler somewhere the crawl may not go: another host, or an excluded address. | Nothing, unless that other place should be indexed. Then add it to `leaf_patterns` for a single page, to `include_patterns` for a whole site, or as a source of its own. |
...
| `PHI lint (<scope>): N pattern match(es) to review in X of Y document(s). Reports: ...` | The check for protected health information found shapes worth a look, and wrote two reports to the folder you ran the build from. | Open the text report and look at each line it names. A clean report is not evidence of anything. A person still decides what is safe to publish. |
...
| `NOTICE   : output '<type>' includes local content; check before publishing.` | An output opted in to content from a `local` source. | Check the file before it leaves your computer. This only happens when you set `include_local: true`. |
...
## When a build has to run on your own computer

Most builds can run on a schedule on GitHub. Two kinds cannot:

- A `local` source. The folder is on your computer, and a cloud runner cannot see it.
- A `youtube` source, the first time. YouTube refuses caption requests from cloud-provider addresses. Build once on your own computer, commit the store the build writes under `cache_dir/youtube/`, and a scheduled build reads the transcripts from there without asking YouTube for anything. Name a visible folder as `cache_dir` for that, since the default `.kb_cache` is usually ignored by git.

[How to run a weekly build](run-a-weekly-build.md) covers both commands, and [how to deploy](deploy.md) sets the choices side by side.
...
* [Troubleshooting](../troubleshooting.md): known failures, including wrong titles and refused sites.
