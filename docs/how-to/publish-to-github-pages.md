...
The workflow asks for as little as it can. Understand the permissions before you approve them.
...
Publishing goes through GitHub's official actions (`actions/configure-pages`, `actions/upload-pages-artifact`, `actions/deploy-pages`) and no others. A third-party publishing action would need a token of its own, and the token is the thing worth protecting.
