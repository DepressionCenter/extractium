...
|---|---|---|
| [Val Town](../../examples/mcp/valtown/README.md) | You want the shortest path: paste six files in the browser, or one Python command with a token. Nothing to install. | Holds the whole index file in memory. Keyword search out of the box; the full hybrid search if you point it at an embedding service. |
