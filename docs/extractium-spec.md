...
| `source_label` | The name a reader sees for the source, from its required `label` setting. At most 60 characters, never empty. Names the source's entry in `llms.txt` and its index file under `llms/` |
