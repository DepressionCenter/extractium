...
| `document_url_patterns` | No | Class attribute. Regular expressions for addresses on the handler's host that serve a document file without naming its extension, such as a portal's attachment download. While a `web` source reads documents, such an address is fetched as a file and read, and the same pattern in the handler's exclude lists is set aside. Left out, an address is a document only when its extension says so. |
