...
| A data repository builds with a named release of the tool | `examples/data-repo/.github/workflows/build-compendium.yml`, `EXTRACTIUM_REF` | The template installs a release tag, `v0.1.0`, rather than `main`, so a change merged here never reaches an adopter's scheduled build unannounced. Pinned in `tests/test_operations.py`. |
...
| Package | Used for | License |
|---|---|---|
| [Tree-sitter](https://tree-sitter.github.io/tree-sitter/) and thirteen language grammars (`code` extra) | Parse a repository's source files so a search can find a definition and what calls it, never copying a source body. | MIT, each |
