...
- The check does not read images or PDFs. Neither does the build, so nothing from them reaches an output, but a folder holding them is not covered by the report. A Word, OpenDocument, or RTF file is read only when a source's `read_documents` setting is on, and its text, including the file's own keywords and description, is then checked like any page.
...
| [youtube-transcript-api](https://github.com/jdepoix/youtube-transcript-api) (`youtube` extra) | Fetches a video's caption track. | MIT |
