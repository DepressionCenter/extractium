...
| A repository archive is read in memory and never extracted to disk | `extractium/sources/github_client.py`, `_files_from_archive` | Only regular files are read, archive paths are compared and never joined onto a real directory, and a path that climbs out matches nothing. `tests/test_github_client.py` covers a symbolic link, a traversal path, an oversized archive, and content that is not text. |
