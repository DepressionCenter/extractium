...
Summary: The security, privacy, and accessibility posture of Extractium:
the controls that exist and where they live in the code, the evidence for
each, the dependency licenses, the known gaps, and what still needs
...
This page states what Extractium™ does to keep content, credentials, and readers safe, and where each control lives so a reviewer can check it. It also lists the known gaps, the dependencies with their licenses, and what no code review can settle. It claims nothing beyond what the repository shows. Read it before you publish anything, and again before you point the tool at content that is not already public.
...
| No absolute path from your disk reaches an output | `extractium/sources/local.py`, `relative_url` | Every local document's URL is `local:` plus a path relative to the source folder. Pinned in `tests/test_source_local.py`. |
...
| The crawl waits between requests to a site | `delay_seconds` setting, default 0.5 seconds, enforced per host by the session every source shares, so sources and workers running at once cannot add up to a faster rate | Documented in the [configuration reference](configuration.md); `HostPacer` in `extractium/core/transport.py`, with its tests in `tests/test_transport.py`. |
| A page is retried under a browser's identity only when you have claimed the site | `extractium/core/fetch.py`, `BROWSER_USER_AGENT` and `fetch` | The retry happens only with `respect_robots_txt: false`, which is your statement that you own the sites in scope. A page that answers 401, 403, or 429 to the configured User-Agent is requested once more as a browser, both attempts are reported, and a 404 or a 5xx is never retried. With the setting at its default, no such retry exists. Covered in `tests/test_web_source.py`. |
| A site behind bot protection is read without the crawler pretending to be something else | `extractium/core/transport.py`, `AutoSession` | Only the TLS handshake is browser-shaped; the `User-Agent` sent over it is the crawler's own, and the retry sends exactly the headers the first request sent. `tests/test_transport.py` checks the headers of the retry; the measurement in [reading a site behind bot protection](bot-protection-transport.md) shows the sites serve that combination. `robots.txt` is fetched and obeyed as before, and a `403` without the challenge header is never retried. |
| The transport a host was served over is on record | The same module, `PLAIN_REPORT`, `BROWSER_REPORT`, `summary_lines` | Reported once per host in the log as it is decided, and listed again in the build summary, so the choice is never silent. Pinned in `tests/test_transport.py`. |
| Nothing from the browser session reaches an output or the cache | `extractium/core/fetch.py`, `_store_fetched_page` | The cache stores page text and the `ETag` and `Last-Modified` validators only; cookies live and die with the session. `tests/test_transport.py` checks that a challenge page is never cached and that no cookie reaches the cache metadata. |
...
| A page taken down leaves the published index on the next build | `extractium/config.py`, `DEFAULT_REBUILD`; `extractium/core/retain.py` | The default rebuild mode publishes exactly what was read. In incremental mode a page is carried forward only from a web crawl, only while no server confirmed it gone with a `404` or `410`, and only while its source is still configured. Covered in `tests/test_retain.py`. |
| The build manifest holds no local content | `extractium/core/retain.py`, `pages_of` | The manifest beside the cache records every published section so a later build can keep a page it did not reach. Sections read from a local folder are left out, because a data repository commits its cache folder. `tests/test_retain.py` checks a local marker never reaches the file. |
| An Open Knowledge Format folder mirrors the compendium, and deletes only what the tool wrote | `extractium/adapters/okf.py`, `prune_bundle` | A concept file for a page no longer in the compendium is removed only when its `generated.by` field names this tool; a file added by hand is never touched, and every removal is named in the build log. Covered in `tests/test_adapter_okf.py`. |
| A redirect cannot carry the crawl off the site either | `extractium/sources/web.py`, `_redirected_out_of_scope` | Where every request lands is checked against the same scope and the handlers' own rules, so a page that redirects to another host or to an excluded address is skipped with the landing address named, rather than indexed under the site's address. On a cloud runner this also keeps a redirect from reaching an internal address the crawl was never allowed to ask for. A handler's own rewrite of a request, such as a blob page fetched from the raw host, is not a redirect. `tests/test_seed_handling.py` covers a redirect off the host, within the host, and to an excluded address. |
...
| Lock files, dotfiles, and anything holding a credential are never downloaded from a repository | `extractium/sources/github_files.py` | `.env`, private keys, certificates, binaries, lock files, every dot-prefixed folder or file, and vendored, generated, and test folders are filtered from the inventory before any request, so a settings file or an editor folder that holds a token by mistake is never fetched. `.env.example`, `.gitmodules`, and the root workflow files are the deliberate exceptions. Covered in `tests/test_github_files.py` and `tests/test_source_github_api.py`. |
...
| Nothing is parsed or unpacked to read a deposit's contents | `extractium/sources/dspace.py`, `_text_for` | Only the plain text the repository extracted when the file was deposited is read. The document readers below are not used for deposits, and there is no PDF or archive reader in the build. |
| A document file is read in memory, with a size ceiling and no document type declaration | `extractium/readers/office.py`, `_raw_part`, `_xml_part`; `extractium/readers/slides.py`; `extractium/readers/documents.py`, `read_document` | A Word, PowerPoint, or OpenDocument file is a zip archive of XML. Each part is read no further than a 50,000,000-byte ceiling however large the entry claims to be, so a compressed bomb is refused rather than unpacked; any part carrying a `DOCTYPE` or `ENTITY` declaration is refused before the parser sees it, which is the only route for an entity expansion or an external entity, so the standard parser is safe to use and no XML library is added; the whole file is refused above 20,000,000 bytes; and nothing is ever written to disk. The format is decided from the first bytes, never from the name or the declared content type. The binary `.doc` and `.ppt` formats, which have no safe reader, are refused by name. A slide part is reached only through a relationship whose target stays inside the archive; one pointing outside it or at a part that is not a slide is ignored. The Word, OpenDocument, RTF, and slide readers use the standard library only. Covered in `tests/test_readers.py`. |
| A PDF is read in a child process that is killed when it runs too long | `extractium/readers/isolated.py`, `Worker`; `extractium/readers/pdf.py`, `read_pdf` | The PDF library's advisory history is a run of resource-exhaustion fixes on malformed input, and a thread cannot be stopped from outside, so the reader runs in a separate process with a 30-second limit per file. A file that runs past it is skipped and the child is killed and replaced; a child that dies is reported and replaced. On POSIX the child also caps its address space at 2,000,000,000 bytes; Windows has no such cap, so there the time limit and the 20,000,000-byte file ceiling are the controls. At most 500 pages of one file are read. An encrypted PDF is refused and no password is tried; a PDF whose pages hold no text is refused as likely scanned images. Only a type name and a message cross the pipe from the child, never a traceback or an object. Covered in `tests/test_readers.py`. |
| A document fetched from a website is checked before it is stored | `extractium/core/fetch.py`, `fetch_bytes`, `_store_fetched_document` | A document file is fetched only when a source's `read_documents` setting is on and the link is in scope. An answer that is a web page, such as a sign-in page, is refused with the landing address named, and an answer over the ceiling is refused before anything is written. Its links, if any, are never followed. Covered in `tests/test_core_fetch.py` and `tests/test_web_source.py`. |
| A Google file that is not shared is skipped, never signed in to | `extractium/sources/google_docs.py`; `extractium/core/fetch.py`, `_store_fetched_page` | Only a file's export address is requested, and only through a leaf or include pattern the operator wrote. A private file answers with a sign-in page, which fails the content-type check and is reported with the landing address on `accounts.google.com`. No credential exists in the build to retry with. Forms, Drive folders, and the sign-in host are on the handler's exclude list. Covered in `tests/test_site_google_docs.py`. |
...
| A typed answer cannot become a setting | `extractium/init.py`, `yaml_text` and `checked_settings_text` | The `init` command quotes every value it writes as a JSON string, which is valid YAML, so a name holding a colon, a quotation mark, or a line break stays one value. The written text is loaded through the settings checks before it is saved, an existing file is never replaced without `--force`, and a website address must be `http` or `https`. `tests/test_init.py` writes hostile names and reads them back. |
| The build scripts download a named release, never a moving branch, unless told to | `run.sh`, `run.bat` | "latest" is resolved to the newest release's tag through the redirect GitHub serves at `releases/latest`, and the download is that tag through git or the release archive. A branch is used only when `EXTRACTIUM_REF` names one. The Python fallback for an archive extracts with the data filter where the interpreter has it. `tests/test_operations.py` pins the lookup and the fallbacks. |
...
| The browser handshake comes from one vetted, pinned package | `pyproject.toml`, `requirements-lock.txt`: `curl_cffi` 0.16.3 | MIT licensed; its only dependencies are `cffi` and `certifi`; wheels for Windows, macOS, and Linux on every architecture this project supports; released 2026-09-02 and maintained. It bundles its own build of libcurl, a native binary, which is why it is pinned with hashes rather than by version alone. Its native library is loaded only when a site is actually challenged. |
| Every action a workflow uses is pinned to a commit digest | The three workflows under `.github/workflows/` and `examples/data-repo/.github/workflows/` | Each `uses:` line names a full commit digest with the release in a comment, so a tag moved under us changes nothing. `tests/test_operations.py` refuses any other form. |
| A data repository builds with a named release of the tool | `examples/data-repo/.github/workflows/build-compendium.yml`, `EXTRACTIUM_REF` | The template installs a release tag, `v0.2`, rather than `main`, so a change merged here never reaches an adopter's scheduled build unannounced. Pinned in `tests/test_operations.py`. |
...
| An AI agent is told to treat indexed text as evidence, never as instructions | `docs/using-a-compendium.md` | The rule is stated in the page an agent is pointed at. |
...
- The browser handshake is a native library in the supply chain. `curl_cffi` ships its own patched libcurl. It is pinned by hash and loaded only when a site challenges the crawler, but it is still compiled code this project did not build. An organization that will not run it sets `transport: plain` and reads challenged sites not at all.
- The check does not read images. Neither does the build, so nothing from them reaches an output, but a folder holding them is not covered by the report. A Word, OpenDocument, RTF, PDF, or slide file is read only when a source's `read_documents` setting is on, and its text, including the file's own keywords and description, is then checked like any page. A scanned PDF holds no text a reader can see, so nothing from it is checked or indexed.
- Text a repository extracted from a deposited document is not scanned by default. The default `phi_lint: local` setting covers content that was never published, and a deposit in a public repository was published deliberately. But extracted text from a research poster is exactly where a stray identifier is most likely to sit, so set `phi_lint: 'all'` on any build with a `dspace` source. The setting exists. Choosing it is up to you.
...
Every license below was read from the package's own metadata or license file, and each is compatible with the GNU General Public License v3.0 or later that this project carries.

Runtime dependencies, installed with the package:

| Package | Used for | License |
|---|---|---|
| [requests](https://requests.readthedocs.io/) | HTTP client. Fetches every page, repository file, and caption listing. | Apache-2.0 |
| [curl_cffi](https://github.com/lexiforest/curl_cffi) | HTTP client with a browser-shaped TLS handshake. Used only when a site answers a bot-protection challenge, with the crawler still naming itself. Bundles its own build of libcurl. | MIT |
| [Beautiful Soup 4](https://www.crummy.com/software/BeautifulSoup/) | HTML parser. Reads each fetched page so the site handlers can find its title and content. | MIT |
| [Sentence Transformers](https://sbert.net/) | Runs the embedding model over every window once per build. | Apache-2.0 |
| [NumPy](https://numpy.org/) | Array arithmetic for the vectors, the near-duplicate collapse, and the calibration statistics. | BSD-3-Clause, with bundled components under 0BSD, MIT, Zlib, and CC0-1.0 |
| [Python-Markdown](https://python-markdown.github.io/) | Renders Markdown files from a local folder or a repository into HTML before chunking. | BSD-3-Clause |
| [PyYAML](https://pyyaml.org/) | Reads the settings file and writes the front matter of the Open Knowledge Format output. | MIT |
| [BAAI/bge-small-en-v1.5](https://huggingface.co/BAAI/bge-small-en-v1.5) | The embedding model every build and every client use, downloaded on the first build. | MIT |
...
| [yt-dlp](https://github.com/yt-dlp/yt-dlp) (`whisper` extra, not in the lock file) | Downloads a video's audio track when YouTube refuses its captions. Called through its Python interface with a fixed format string, a size ceiling, no playlist, no post-processor, and no JavaScript runtime, so nothing it finds on a page is run; its own log is silenced and only this project's words reach the build log. Confirmed to install and run on Python 3.14 on Windows on 2026-09-16. Its release cadence is monthly because YouTube changes its pages; an old copy fails to download rather than doing anything else. | Unlicense |
| [faster-whisper](https://github.com/SYSTRAN/faster-whisper) (`whisper` extra, not in the lock file) | Transcribes a downloaded audio file with Whisper's `base.en` model on the CPU. Brings CTranslate2 (MIT), PyAV (BSD-3-Clause), an ONNX runtime for its silence filter (MIT), and the Hugging Face hub client the embedding model already needs. The model weights, about 75 MB, download once from the Hugging Face hub into the same cache as the embedding model. Confirmed to install and run on Python 3.14 on Windows on 2026-09-16. | MIT |
| [pypdf](https://github.com/py-pdf/pypdf) (`pdf` extra, carried by the lock file) | Reads the text, bookmarks, and properties of a PDF file, inside a child process with a time limit. Pure Python, no compiled part. Checked on 2026-09-15, and again the same day against the GitHub advisory database: thirty published advisories, all resource-exhaustion cases on malformed files (long runtimes, infinite loops, large memory), none code execution; the newest, published 2026-09-02, is fixed in 6.16.1, and the pin, 6.18.1, is the newest release and past every one. Check again whenever the lock file is regenerated. | BSD-3-Clause |
| [YAKE](https://github.com/INESCTEC/yake) (`keywords` extra, carried by the lock file) | Proposes the candidate phrases the keyword step ranks, from word statistics of a section's own text. Pure Python; its stop-word lists ship inside the wheel; nothing it does reaches the network. Checked against the dependency gate on 2026-09-15: Python 3.10 or newer, released 2026-02, no published advisory for it or for the three packages it adds, segtok (MIT), tabulate (MIT), and jellyfish (MIT, with wheels for Windows, macOS, and Linux). The other packages it names, click, networkx, numpy, and regex, were in the lock file already through the embedding stack. Its metadata names the LGPL v3 as its license and its classifier the GPL v3; both are compatible with this project's license. Check again whenever the lock file is regenerated. | LGPL-3.0 |

Development dependencies (`dev` extra and the lock file):

| Package | Used for | License |
|---|---|---|
| [pytest](https://docs.pytest.org/) and pytest-cov | Run the test suite and measure its coverage. | MIT, each |
| [uv](https://docs.astral.sh/uv/) | Generates the hash-checked lock file the run scripts and the scheduled build install from. | Apache-2.0 or MIT |

Used by the examples only:

| Package | Used for | License |
|---|---|---|
| [@huggingface/transformers](https://huggingface.co/docs/transformers.js) and the ONNX Runtime it brings | Run the same embedding model in the Node search server. | Apache-2.0 and MIT respectively |
...
| Item | Status |
|---|---|
| Test suite | 1,786 Python tests passing as of 2026-09-12, with one skipped, plus 39 Node tests for the JavaScript client, 24 for the local Node MCP server, 19 for the shared MCP core, 13 for the Val Town example, and 14 for the Cloudflare example. |
...
| Penetration testing | Not done. The tool itself is a local program, but the two hosted examples are services once deployed; each was exercised against the golden compendium with the refusals above, the Val Town one on the live platform, and nothing more. |


## Conclusion

The posture is: publish public content, keep local content out by default, install only what was locked, and ask for the least access a scheduled build needs. The gaps above are named rather than hidden. Update this page in the same change set as any work that alters one of them.
