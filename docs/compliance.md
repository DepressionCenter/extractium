...
| A page is retried under a browser's identity only when the operator has claimed the site | `extractium/core/fetch.py`, `BROWSER_USER_AGENT` and `fetch` | The retry happens only with `respect_robots_txt: false`, which is the operator's statement that they own the sites in scope. A page that answers 401, 403, or 429 to the configured User-Agent is requested once more as a browser, both attempts are reported, and a 404 or a 5xx is never retried. With the setting at its default, no such retry exists. Covered in `tests/test_web_source.py`. |
