...
| Operations | `run.sh`, `run.bat`, `requirements-lock.txt`, `.github/workflows/build-compendium.yml`, `examples/data-repo/` | One command builds locally on either platform from a hash-checked lock file. A script saved on its own first downloads the latest release, through git or as an archive, and hands over to the copy of itself inside the download. With no settings file it runs the `init` command and limits the first build to 25 pages. The workflow runs weekly and on a button press, reuses the crawl cache between runs, and publishes through the official GitHub Pages actions only. The data-repository template is what an organization copies for its own content. |
