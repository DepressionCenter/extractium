...
| Document readers | `extractium/readers/documents.py`, `office.py`, `rtf.py`, `pdf.py`, `isolated.py` | Turn a Word, OpenDocument, RTF, or PDF file's bytes into Markdown-like text with its headings, lists, and tables, and read the title, subject, keywords, and description from the file's properties. The Word, OpenDocument, and RTF readers are standard library only. The PDF reader uses `pypdf`, the optional `pdf` extra, and runs in one reusable child process with a time limit per file, killed and replaced when a file runs past it; its headings come from the file's bookmarks with the page number appended, or from the page numbers alone. The format is decided from the first bytes, never from the name; the binary `.doc` format is refused by name; an archive entry over its ceiling and any XML part carrying a document type declaration are refused before parsing; an encrypted PDF and one with no text are refused with the reason. The `web`, `local`, and `github_api` sources hand bytes here when their `read_documents` setting is on. |
