...
| Video source | `extractium/sources/youtube.py`, `youtube_client.py`, `youtube_pages.py` | Indexes what is said in a video: each stretch of a caption track becomes one section addressed at the moment it begins, so a citation opens the video at the quoted words. A channel may be named by handle, by address, or by id. Listings come from the Data API when a key is set and from YouTube's own pages when none is. The page path stops where `robots.txt` does, at the newest hundred of a listing, and says so. A channel's uploads and its playlists are both read, and a video in a playlist that another channel published is left out. Everything read is stored under `cache_dir` and meant to be committed, because YouTube refuses caption requests from cloud-provider addresses. The caption library is the optional `extractium[youtube]` install. When it is refused and the optional `extractium[whisper]` install is present, `youtube_audio.py` downloads the video's audio with yt-dlp and transcribes it with Whisper's smallest English model on the CPU, and the result is stored like any other transcript. |
