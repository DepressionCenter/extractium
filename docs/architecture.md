...
| Adapters | `extractium/adapters/container.py`, `llmstxt.py`, `sqlite_out.py`, `okf.py` | The container writer produces the version 4 file, compressed through gzip when the output asks. The llms.txt writer produces `llms.txt`, an index of the sources, and the `llms/` folder, one index file per source listing its pages with a description of each; a source over 500 entries splits into one file per category or repository. It lists no code records, and it removes an index file it wrote for a source that is gone. The SQLite writer produces `compendium.sqlite`, the same content in tables a SQL consumer can query. The Open Knowledge Format writer produces the `okf/` folder, one Markdown file per page with the front matter that format defines, and keeps the folder in step with the compendium by removing only files it wrote itself. `extractium/adapters/base.py` holds the output folder helper, the text helpers the Markdown writers share, the one-record-per-page grouping with the rule for describing a page (its own summary, then its keywords, then the opening of its text), and the local-content guardrail every adapter goes through. |
