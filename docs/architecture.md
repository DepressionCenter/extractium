...
| Scoring | `extractium/core/embed.py`, `dedup.py`, `bm25.py`, `calibration.py` | The embedding pass, the near-duplicate collapse, the keyword statistics, and the calibration statistics, which include what a fixed list of unrelated questions scores in the finished corpus, the figure a search client sets its relevance floor from. `core/build.py` runs them in order. The embedding library is imported only when embedding runs, so nothing that merely reads an index loads it. |
| Keywords | `extractium/core/keywords.py` | Names every section with keywords and every page with tags after compaction. The optional YAKE extractor proposes candidate phrases from a section's text, the embedding model ranks them by closeness to the section's mean window vector, and the keywords most of a page's sections share become its tags after the source's categories. Results are stored under the cache folder by section id and text digest, so a rebuild names afresh only what changed. `core/build.py` runs it when the command line hands it one; a library caller that passes none gets sections without keywords. |
...
| Light compendium | `extractium/core/light.py` | `build_light_compendium` turns that result into a second, small `Compendium` for the light container: one section per page, holding the page's description and keywords, with its own windows, vectors, and statistics. It skips the chunker and the near-duplicate step, so no page is lost for a short or shared description. The command line runs it once, and only when a container output is configured. |
...
| Video source | `extractium/sources/youtube.py`, `youtube_client.py`, `youtube_pages.py` | Indexes what is said in a video: each stretch of a caption track becomes one section addressed at the moment it begins, so a citation opens the video at the quoted words. A channel may be named by handle, by address, or by id. Listings come from the Data API when a key is set and from YouTube's own pages when none is. The page path stops where `robots.txt` does, at the newest hundred of a listing, and says so. A channel's uploads and its playlists are both read, and a video in a playlist that another channel published is left out. Everything read is stored under `cache_dir` and meant to be committed, because YouTube refuses caption requests from cloud-provider addresses. The caption library is the `youtube` extra and the audio packages are the `whisper` extra; both are pinned in the lock file, so every scripted build has them. When a caption request is refused, `youtube_audio.py` downloads the video's audio with yt-dlp and transcribes it with Whisper's smallest English model on the CPU, and the result is stored like any other transcript. |
...
| Adapters | `extractium/adapters/container.py`, `llmstxt.py`, `sqlite_out.py`, `okf.py` | The container writer produces two version 4 files, compressed through gzip unless the output says otherwise: a light one with one short section per page, built by `extractium/core/light.py` from each page's description and keywords, and a full one with the text of every section. Both leave code records out. The llms.txt writer produces `llms.txt`, an index of the sources, and the `llms/` folder, one index file per source listing its pages with a description of each; a source over 500 entries splits into one file per category or repository. It lists no code records, and it removes an index file it wrote for a source that is gone. The SQLite writer produces `compendium.sqlite`, everything the build read, code records included, in tables a SQL consumer can query; [the SQLite database](sqlite-database.md) draws them. The Open Knowledge Format writer produces the `okf/` folder, one Markdown file per page with the front matter that format defines, and keeps the folder in step with the compendium by removing only files it wrote itself. `extractium/adapters/base.py` holds the output folder helper, the text helpers the Markdown writers share, the one-record-per-page grouping with the rule for describing a page (its own summary, then its keywords, then the opening of its text), the local-content guardrail every adapter goes through, and the routine under it that drops any set of sections and rebuilds the windows, vectors, and statistics over what remains, which is also how the full container leaves code out. |
...
| Hosted assistant prompts | `examples/wrappers/` | Two plain-text system prompts: one for a platform that can browse but not call tools, which walks from `llms.txt` to a source's index file to the page, and one for a platform connected to a hosted search server. |
| Operations | `run.sh`, `run.bat`, `requirements-lock.txt`, `tools/lock_without_parsers.py`, `.github/workflows/build-compendium.yml`, `examples/data-repo/` (with `.gitlab-ci.yml`) | One command builds locally on either platform from a hash-checked lock file. The scripts prefer a standard Python over a free-threaded one, which cannot use the parsers' wheels, and on a machine with only the free-threaded build they install from a copy of the lock file without the parsers, every other hash still checked. A script saved on its own first downloads the latest release, through git or as an archive, into a folder whose name is not the package's, and hands over to the copy of itself inside the download. Builds run through the command the install places in the environment, because the module form would import a folder in the working directory ahead of the installed package. With no settings file it runs the `init` command and limits the first build to 25 pages. The workflow runs weekly and on a button press, reuses the crawl cache between runs, and publishes through the official GitHub Pages actions only. The GitLab pipeline in the template does the same on an institution's own runners, keeps the output as an artifact, and publishes to Pages only when a variable says so. The data-repository template is what an organization copies for its own content. |
...
Decision: the Tree-sitter grammars are the `extractium[code]` extra and the caption library is the `extractium[youtube]` extra. Neither is a runtime dependency of the package, but the lock file the build scripts and the scheduled workflow install from is generated with both extras, so every scripted build carries the parsers and the caption library. A developer install without them still records every source file by name, language, and length, and still reads transcripts already stored.

Why: the parser set is seventeen small packages with native wheels, and a build that reads a GitHub account without them reports every language as unparsed, which is what happened on the first full build from the standalone script. The caption library is one small package, and a scripted build without it stops at the first video that has no stored transcript, which is what happened on the first scripted build of a settings file with a video source. The scripts are how a build is made on a person's own computer, and that is the only kind of machine YouTube answers, so a lock without the library left captions unreachable from the path most people take. Keeping both an extra of the package keeps a computer that cannot install them able to build everything else; putting both in the lock keeps the scripted path complete.
...
Why: two sources often overlap without meaning to: a website and a section of it, or a portal and a short link into one of its articles. Indexing the page twice would double its weight in every search and list it twice in the `llms/` index files. A section of a site that is already crawled belongs to that crawl as a second `seed_urls` entry, not to a source of its own.
...
### 17. Keywords come from the text and the vectors, not from a language model

Decision: every section is named with keywords and every page with tags by a statistical extractor and the embedding model the build already loads. No language model is called, the step is on by default, its results are cached by section, and the extractor is an optional extra a build can do without.

Why: the enrichment fields had been reserved for a pass needing a local instruct model and a graphics card, which most builds of this tool will never have. Keywords are the part of that pass that needs neither: the extractor works from word statistics alone, and ranking its candidates by the section's own vector reuses work the build has already paid for. A knowledge base with keywords in every output is more useful to a reader and to a model today than one waiting for a summary pass. The summary field stays reserved.

Specification: sections 3.4 and 10.
