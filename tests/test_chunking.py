"""
Summary: Characterization tests pinning the current parent/child
(small-to-big) chunking behavior of build-kb-index.py: split_into_parents
(headings vs. no headings, long-section hard split), split_parent_into_children
(fixed-step window, overlap, forward-progress guarantee), and
build_parent_and_child_chunks (per-page local pid indices).
...
def test_split_into_parents_no_headings_yields_single_parent(reference, fixtures_dir):
    ...
    title = reference.get_title(soup, url)

    parents = reference.split_into_parents(title, node, url)
...
def test_split_into_parents_with_headings_yields_one_parent_per_section(reference, fixtures_dir):
...
def test_split_into_parents_hard_splits_long_section_without_newlines(reference, fixtures_dir):
    ...
    assert all(len(p["x"]) <= reference.CHUNK_MAX_CHARS for p in parents)
    ...
    assert len(parents[0]["x"]) == reference.CHUNK_MAX_CHARS
    assert len(parents[1]["x"]) == reference.CHUNK_MAX_CHARS
    assert len(parents[2]["x"]) < reference.CHUNK_MAX_CHARS
...
def test_split_parent_into_children_short_parent_is_single_child(reference):
    parent = _make_parent("Short body text well under the child chunk threshold.")
    children = reference.split_parent_into_children(parent)
    assert children == [dict(parent)]


def test_split_parent_into_children_window_stepping_and_overlap(reference):
    ...
    step = max(
        reference.CHILD_CHUNK_MAX_CHARS - reference.CHILD_OVERLAP_CHARS,
        reference.CHILD_CHUNK_MIN_CHARS,
    )
    assert step == 297  # max(350 - 53, 60)

    # Starts at 0, 297, 594, 891; the last window's end clamps to n=1000.
    assert len(children) == 4
    assert all(len(c["x"]) <= reference.CHILD_CHUNK_MAX_CHARS for c in children)
    assert len(children[0]["x"]) == reference.CHILD_CHUNK_MAX_CHARS
    ...
    for i in range(len(children) - 1):
        overlap = reference.CHILD_OVERLAP_CHARS
        assert children[i]["x"][-overlap:] == children[i + 1]["x"][:overlap]


def test_split_parent_into_children_guarantees_forward_progress(reference):
...
def test_build_parent_and_child_chunks_assigns_local_pid_indices(reference, fixtures_dir):
    soup = _soup_from_fixture(fixtures_dir, "tdx_article.html")
    url = "https://teamdynamix.umich.edu/TDClient/210/Test/KB/ArticleDet?ID=1"
    node = reference.extract_content(soup, url)
    title = reference.get_title(soup, url)

    parents, children = reference.build_parent_and_child_chunks(title, node, url)
